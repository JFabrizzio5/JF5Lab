...
@app.get("/")
async def root():
    return {"app": APP_NAME, "desc": "Cobranza automatica B2B. Docs en /docs."}
                ...
                for version in os.listdir(api_path):
                    ...
                    if os.path.isdir(v_dir) and not version.startswith("__"):
                        try:
                            ...
                            # PorCobrar mount: `cobranza` module served at `/cobrar/v1`
                            mount_name = "cobrar" if module_name.lower() == "cobranza" else module_name.lower()
                            app.include_router(route_mod.router, prefix=f"/{mount_name}/{version}", tags=[f"{module_name.capitalize()} {version.upper()}"])
                            try:

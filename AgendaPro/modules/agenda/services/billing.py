"""Billing: Stripe + Conekta checkout con fallback demo."""
...
def _public_url() -> str:
    return os.getenv("APP_PUBLIC_URL", "http://localhost:3055")
...
def create_stripe_checkout(*, amount_mxn: Decimal, description: str, reference: str,
    ...
    if not key:
        url = f"{_public_url().replace(':3055', ':8120')}/agenda/v1/billing/demo-checkout/stripe?ref={reference}&amount={amount_mxn}"
        return url, None
    try:
        import stripe
        stripe.api_key = key
        session = stripe.checkout.Session.create(
            ...
            success_url=success_url or f"{_public_url()}/book/success?ref={reference}",
            cancel_url=cancel_url or f"{_public_url()}/book/cancel?ref={reference}",
...
def create_conekta_checkout(*, amount_mxn: Decimal, description: str, reference: str,
    ...
    if not key:
        url = f"{_public_url().replace(':3055', ':8120')}/agenda/v1/billing/demo-checkout/conekta?ref={reference}&amount={amount_mxn}"
        return url, None
    try:
        import conekta
        conekta.api_key = key
        ...
        order = conekta.Order.create({
            ...
            "customer_info": {
                "name": customer_name or "Cliente AgendaPro",
                "email": customer_email or "cliente@agendapro.mx",

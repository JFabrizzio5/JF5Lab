...
export const useSession = defineStore('session', () => {
  ...
  function set(t) {
    ...
    brandColor.value = t.brand_color || '#7c3aed'
    localStorage.setItem('pulsomx_session', JSON.stringify({
  ...
  function restore() {
    try {
      const raw = localStorage.getItem('pulsomx_session')
      if (raw) {
  ...
  function clear() {
    ...
    localStorage.removeItem('pulsomx_session')
  }
